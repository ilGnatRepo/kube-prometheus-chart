...
## Collect Charts Togather.

Due to network reasons, it's difficult to add the Helm repo [prometheus-community](https://prometheus-community.github.io/helm-charts). The related Docker images have the same problem, so I try to collect the charts and Docker images together locally and install them.

```console
# Find the prometheus-community repo in github, pull the code and access the kube-prometheus-stack dectionary.
cd prometheus-community/charts/kube-prometheus-stack

# See chart.yaml's denpendencies
sed -n '/dependencies/,$p' Chart.yaml | grep -v condition
"""
	dependencies:
	  - name: crds
	    version: "0.0.0"
...
```console
cat values.yaml | grep -E '(registry:|repository:|tag:)'
"""
	    registry: registry.local:8000
